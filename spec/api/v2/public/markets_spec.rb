  ...
  describe 'GET /api/v2/markets' do
    ...
    let(:expected_keys) do
      %w[id name base_unit quote_unit min_price max_price
         min_amount amount_precision price_precision position state]
    end
    ...
    context 'order_by' do
      it 'filters by quote_unit position' do
        get '/api/v2/public/markets', params: { order_by: :quote_position }
        expect(response).to be_successful
        result = JSON.parse(response.body)

        expect(result.pluck('id')).to eq %w(btcusd ethusd btceth)
      end

      it 'filters by base_unit position descending' do
        get '/api/v2/public/markets', params: { order_by: :quote_position, ordering: :desc }
        expect(response).to be_successful
        result = JSON.parse(response.body)

        expect(result.pluck('id')).to eq %w(btceth btcusd ethusd )
      end
    end

    context 'extended' do
      it 'returns extended response' do
        get '/api/v2/public/markets', params: { extended: true }
        expect(response).to be_successful
        result = JSON.parse(response.body)

        expect(result.first.keys).to include(*%w(base quote ticker))
      end

      it 'returns non extended response' do
        get '/api/v2/public/markets'
        expect(response).to be_successful
        result = JSON.parse(response.body)

        expect(result.first.keys).to_not include(*%w(base quote ticker))
      end
    end
  end
